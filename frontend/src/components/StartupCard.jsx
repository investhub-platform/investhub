...
export function StartupCard({ startup, index }) {
  ...
  const detailPath = startup?.isIdea
    ? `/app/idea/${startup._id || startup.id}`
    : `/app/startup/${startup._id || startup.id}`;
  ...
  return (
    ...
    >
      <Link
        to={detailPath}
        state={{ background: location, startup }}
      ...
      >
        <div className="group flex flex-col h-full bg-[#0B0D10] rounded-[1.5rem] border border-white/5 overflow-hidden transition-all duration-300 hover:border-white/15 hover:shadow-2xl hover:shadow-blue-900/10 hover:-translate-y-1">
          {/* Top Banner Area */}
          <div
            className={`h-24 w-full relative overflow-hidden border-b border-white/5 ${startup.recordType === "idea" || startup.isIdea ? "bg-transparent" : `bg-gradient-to-br ${gradientClass}`}`}
          >
            {/* If this is an idea post and has a cover image, show it as the large banner */}
            {(startup.recordType === "idea" || startup.isIdea) &&
            startup.photoUrl ? (
              <img
            ...
            <div className="absolute -bottom-5 left-5 w-14 h-14 md:w-16 md:h-16 rounded-xl bg-[#0B0D10] border border-white/10 flex items-center justify-center text-xl font-bold text-white shadow-lg overflow-hidden">
              {startup.logo ? (
                ...
                <span className="inline-block w-full h-full flex items-center justify-center text-2xl font-black text-white">
          ...
          <div className="p-5 pt-8 flex flex-col flex-1">
            <div className="flex justify-between items-start mb-2">
              ...
              <div
                className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[10px] font-bold tracking-wider uppercase border ${
            ...
            <div className="flex flex-wrap gap-2 mb-auto">
              {tags.slice(0, 3).map((tag) => (
                <span
                  key={tag}
                  className="text-[11px] font-medium text-slate-300 bg-white/5 border border-white/5 py-1 px-2.5 rounded-full"
                >
                  {tag}
                </span>
              ))}
            ...
            <div className="mb-4">
              <div className="flex justify-between text-xs text-slate-400 mb-2 font-medium">
                <span>
                  Target:{" "}
                  <strong className="text-white">{formatCurrency(goal)}</strong>
                </span>
