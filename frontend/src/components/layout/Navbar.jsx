...
const Navbar = () => {
  ...
  return (
    <header className="fixed top-4 sm:top-6 inset-x-0 z-50 flex justify-center px-3 sm:px-0 pointer-events-none">
      <div ref={navShellRef} className="pointer-events-auto flex items-center justify-between px-4 py-3 bg-white/5 backdrop-blur-lg backdrop-saturate-150 border border-transparent rounded-full shadow-lg w-full max-w-7xl transition-all">
        
        <a href="#hero" onClick={(e) => handleSectionScroll(e, 'hero')} className="flex items-center gap-3 cursor-pointer">
        ...
        <nav className="hidden md:flex items-center gap-8 text-sm font-medium text-slate-300">
          ...
          <a href="#faq" onClick={(event) => handleSectionScroll(event, 'faq')} className="hover:text-white transition-colors">FAQ</a>
      ...
      <AnimatePresence>
        {open && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: -20 }}
            className="pointer-events-auto absolute top-20 left-1/2 transform -translate-x-1/2 w-[92%] max-w-md bg-white/5 backdrop-blur-lg backdrop-saturate-150 border border-transparent rounded-2xl p-4 z-40 md:hidden shadow-lg"
          >
