    ...
    <div className="lg:w-[380px] shrink-0 space-y-6 order-1 lg:order-2">
      {!isPlan && (
        <div className="p-6 md:p-8 rounded-[2rem] bg-gradient-to-b from-[#0B0D10] to-[#020617] border border-blue-500/30 shadow-[0_0_40px_rgba(37,99,235,0.15)] relative overflow-hidden group">
          ...
          {console.log("Startup Owner:", startup?.raw.createdBy)}
          {console.log("Current User ID:", currentUserId)}

          {currentUserId == startup?.raw.createdBy && (
            <div className="text-sm text-emerald-400 font-bold mb-4">
              You are the owner of this startup.
            </div>
          )}
          <div className="space-y-4 mb-6">
            <div>
              <label className="text-xs font-bold tracking-widest uppercase text-slate-500 mb-2 block ml-1">
                Investment Amount
              </label>
          ...
          {currentUserId === startup?.raw.createdBy ? (
            <div className="text-sm text-yellow-400 font-bold mb-4">
        ...
        <div className="space-y-4">
          {detailsRows.map(([label, value]) => (
            ...
            >
              ...
              <span
                className="text-sm font-bold text-white text-right max-w-[60%] truncate"
                title={value}
              >
                {value}
              </span>
      ...
      {startup.recordType === "idea" && startup.startupProfile && (
        <div className="p-6 rounded-[2rem] bg-[#0B0D10] border border-white/5 shadow-xl group cursor-pointer hover:border-white/10 transition-colors">
          ...
          <p className="text-base font-bold text-white mb-1 group-hover:text-blue-400 transition-colors">
            {startup.startupProfile.name}
          </p>
          <p className="text-xs text-slate-400 line-clamp-2">
            {startup.startupProfile.tagline}
          </p>
        </div>
      )}
    </div>
