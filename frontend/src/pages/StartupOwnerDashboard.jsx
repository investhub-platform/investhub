...
const inputClass =
  "w-full px-4 py-3.5 rounded-xl bg-[#1A1D24] border border-white/5 text-white text-sm focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50 transition-all placeholder:text-slate-500 shadow-inner";
...
const StartupOwnerDashboard = () => {
  ...
  const normalizeStartup = (startup) => ({
    ...
    fundingGoal: Number(
      startup.budget || startup.fundingGoal || startup.goal || 0
    ),
    currentFunding: Number(
      startup.currentFunding || startup.raised || startup.capitalRaised || 0
    ),
    investorRequests: startup.investorRequests || [],
    logoUrl: resolveAssetUrl(
      startup.ImgURL || startup.logo || startup.logoUrl || ""
    ),
    logoInitials:
  ...
  const handleRequestStatus = async (startupId, request, status) => {
    try {
      ...
      if (status === "withdrawn") {
        ...
        (request?.status === "pending_founder" ||
  ...
  const handlePlanSubmit = async (e) => {
    ...
    if (!planFormData.title.trim() || !planFormData.description.trim())
      return setPlansError("Title and description are required.");
    setPlanSaving(true);
    setPlansError("");
    ...
    try {
      ...
      if (planFormData.category === "Other")
        form.append("customCategory", planFormData.customCategory || "");
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="mb-10">
              ...
              <p className="text-slate-400 text-sm md:text-base font-medium">
                Manage your startups, pitch ideas, and handle investor
                relations.
              </p>
            </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8 pb-6 border-b border-white/5">
                    <div>
                      ...
                      <p className="text-sm text-slate-400 mt-1">
                        Manage active startups and sub-ideas.
                      </p>
                    </div>
                    <button
                      onClick={() =>
                        ...
                        {loading ? (
                          ...
                          <div className="text-center py-20 bg-[#0B0D10]/80 border border-white/5 rounded-[2rem] flex flex-col items-center justify-center">
                            ...
                            <p className="text-sm text-slate-400">
                              Click &quot;Add Startup&quot; to create your first
                              company profile.
                            </p>
                          </div>
                        ) : (
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8 pb-6 border-b border-white/5">
                    <div>
                      ...
                      <p className="text-sm text-slate-400 mt-1">
                        Post investor plans or funding needs not tied to a
                        specific startup.
                      </p>
                    </div>
                    <button
                    ...
                    >
                      {isPlanFormOpen ? (
                        ...
                        <>
                          <Plus className="w-4 h-4" /> Post Investor Plan
                        </>
                      )}
                    </button>
                  </div>
                    ...
                    >
                      <div>
                        <h3 className="text-xl font-bold text-white mb-1">
                          {editingPlanId
                            ? "Edit Plan"
                            : "Post New Investor Plan"}
                        </h3>
                        <p className="text-sm text-slate-400">
                          Share your investor plan to attract potential
                        ...
                        <div>
                          ...
                          {planFormData.category === "Other" && (
                            <input
                              ...
                              value={planFormData.customCategory}
                              onChange={(e) =>
                                setPlanFormData((p) => ({
                                  ...p,
                              ...
                              placeholder="Specify custom category (required)"
                            />
                          )}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            placeholder="Paste raw text from your pitch deck (optional)"
                          />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <UploadField
                          ...
                          onChange={(e) =>
                            setPlanPhotoFile(e.target.files?.[0] || null)
                          }
                        ...
                        <UploadField
                          ...
                          onChange={(e) =>
                            setPlanPitchFiles(Array.from(e.target.files || []))
                          }
                          files={planPitchFiles}
                          onClear={() => setPlanPitchFiles([])}
                          helperText="PDF, DOCX, Images"
                        />
                      </div>
                      ...
                      <div className="flex justify-end gap-3 pt-4 border-t border-white/5">
                        <button
                          ...
                          className="px-6 py-3 rounded-xl bg-white/5 hover:bg-white/10 text-white font-bold text-sm transition-colors"
                        >
                          Cancel
                        </button>
                        <button
                          type="submit"
                          disabled={planSaving}
                          className="px-6 py-3 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm transition-colors shadow-[0_0_15px_rgba(37,99,235,0.3)] disabled:opacity-50"
                        >
                          {planSaving
                            ? "Saving..."
                            : editingPlanId
                  ...
                  {plansLoading ? (
                    ...
                    <div className="text-center py-20 bg-[#0B0D10]/80 border border-white/5 rounded-[2rem] flex flex-col items-center justify-center">
                      ...
                      <p className="text-sm text-slate-400">
                        Post an investor plan or funding mandate to the network.
                      </p>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-20">
                      {plans.map((plan) => (
                        <div
                          ...
                          className="p-6 md:p-8 rounded-[2rem] bg-[#0B0D10] border border-white/5 shadow-xl flex flex-col group"
                        >
                          ...
                          {plan.ImgURL && (
                            <div className="w-full h-40 rounded-xl overflow-hidden mb-6 border border-white/5">
                              <img
                                src={resolveAssetUrl(plan.ImgURL)}
                                alt={plan.title}
...
function StartupManageCard({
  ...
  const handleIdeaSubmit = async (e) => {
    ...
    if (!ideaFormData.title.trim() || !ideaFormData.description.trim())
      return setIdeasError("Title and description are required.");
    setIdeaSaving(true);
    setIdeasError("");
  ...
  return (
    <motion.div
      ...
      className="rounded-[2rem] bg-[#0B0D10] border border-white/5 shadow-xl overflow-hidden"
    >
      {/* Header Row (Accordion Toggle) */}
      <button
        onClick={() => setExpanded(!expanded)}
        className="w-full p-6 md:p-8 flex items-center justify-between gap-4 text-left hover:bg-white/[0.02] transition-colors"
      >
        <div className="flex items-center gap-5 min-w-0">
          ...
          <div className="flex-1 min-w-0 pr-4">
            <div className="flex items-center gap-3 flex-wrap mb-1">
              ...
              <span
                className={`text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-md border ${status.color}`}
              >
                {status.label}
              </span>
              {pendingRequests > 0 && (
          ...
          >
            <div className="px-6 md:px-8 pb-8 pt-2 border-t border-white/5 space-y-8">
                ...
                {!isEditing ? (
                  <>
                    <button
                      ...
                      className="px-5 py-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-xs font-bold text-white transition-colors shadow-sm"
                    >
                      Edit Details
                    </button>
                    <button
                      ...
                      className="px-5 py-2 rounded-xl bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 text-xs font-bold text-red-400 transition-colors shadow-sm"
                    >
                      Delete Startup
                    </button>
                  </>
                  ...
                  <form
                    onSubmit={handleSave}
                    className="w-full space-y-4 bg-[#1A1D24]/50 p-5 rounded-2xl border border-white/5"
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <UploadField
                        ...
                        accept="image/png,image/jpeg,image/webp"
                        onChange={(e) =>
                          setStartupLogoFile(e.target.files?.[0] || null)
                        }
                        files={startupLogoFile ? [startupLogoFile] : []}
                        onClear={() => setStartupLogoFile(null)}
                        icon="image"
                      />
                      <div />
                    </div>
                    <div className="flex items-center justify-end gap-3 pt-2">
                      ...
                      <button
                        type="submit"
                        className="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-xs font-bold text-white transition-colors shadow-[0_0_15px_rgba(37,99,235,0.3)]"
                      >
                        Save Changes
                      </button>
                    </div>
              ...
              <div className="pt-6 border-t border-white/5">
                ...
                {ideasError && (
                  <p className="text-xs font-bold text-red-400 bg-red-500/10 p-3 rounded-lg mb-4">
                    {ideasError}
                  </p>
                )}
                ...
                <AnimatePresence>
                  {ideaFormOpen && (
                    <motion.form
                      ...
                      className="space-y-5 mb-8 rounded-2xl border border-white/10 bg-[#1A1D24]/50 p-6"
                    >
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <label className="block text-xs font-bold uppercase tracking-widest text-slate-400 mb-2 ml-1">
                          ...
                          <input
                            ...
                            placeholder="e.g. Mobile App Expansion"
                            required
                          />
                        </div>
                        <div>
                          ...
                          {ideaFormData.category === "Other" && (
                            <input
                              className={`${inputClass} mt-3`}
                              value={ideaFormData.customCategory}
                              onChange={(e) =>
                                setIdeaFormData((p) => ({
                                  ...p,
                                  customCategory: e.target.value
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <UploadField
                          ...
                          accept="image/png,image/jpeg,image/webp"
                          onChange={(e) =>
                            setIdeaPhotoFile(e.target.files?.[0] || null)
                          }
                        ...
                        <UploadField
                          ...
                          onChange={(e) =>
                            setIdeaPitchFiles(Array.from(e.target.files || []))
                          }
                      ...
                      <div className="flex justify-end gap-3 pt-4 border-t border-white/5">
                        <button
                          ...
                          className="px-5 py-2.5 rounded-xl bg-white/5 text-xs font-bold text-white hover:bg-white/10 transition-colors"
                        >
                          Cancel
                        </button>
                        <button
                          type="submit"
                          disabled={ideaSaving}
                          className="px-5 py-2.5 rounded-xl bg-blue-600 text-xs font-bold text-white hover:bg-blue-500 transition-colors disabled:opacity-50 shadow-[0_0_15px_rgba(37,99,235,0.3)]"
                        >
                          {ideaSaving
                            ? "Saving..."
                            : editingIdeaId
                ...
                {ideasLoading ? (
                  <div className="flex items-center gap-3 text-sm font-medium text-slate-400">
                    <Loader className="w-4 h-4 animate-spin" /> Loading ideas...
                  </div>
                ) : ideas.length === 0 ? (
                  <p className="text-sm font-medium text-slate-500 italic">
                    No specific ideas posted under this startup yet.
                  </p>
                ) : (
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    {ideas.map((idea) => (
                      <div
                        key={idea._id || idea.id}
                        className="p-5 rounded-2xl bg-white/5 border border-white/5 group hover:border-white/10 transition-colors"
                      >
                        ...
                        <div className="flex gap-4 border-t border-white/5 pt-3">
                          <div>
                            ...
                            <p className="text-xs font-bold text-white">
                              {idea.budget ? formatCurrency(idea.budget) : "-"}
                            </p>
                          </div>
                          <div>
                            <p className="text-[9px] font-bold uppercase tracking-widest text-slate-500">
...
function UploadField({
  ...
  return (
    <div className="rounded-xl border border-white/5 bg-[#1A1D24] p-4 shadow-inner">
      <label className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-3 block ml-1">
        {label}
      </label>
      ...
      {helperText && (
        <p className="text-[10px] font-medium text-slate-500 mt-2 ml-1">
          {helperText}
        </p>
      )}
      ...
      {files.length > 0 && (
        <div className="mt-3 space-y-2">
          ...
          <button
            type="button"
            onClick={onClear}
            className="text-[10px] font-bold uppercase tracking-widest text-red-400 hover:text-red-300 ml-1"
          >
            Clear selection
          </button>
        </div>
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-1">
            ...
            <div
              className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md border text-[10px] font-bold uppercase tracking-widest w-fit ${statusColors[normalizedStatus] || statusColors.pending_founder}`}
            >
          ...
          {(canFounderDecide || canWithdraw) && (
            <div className="flex flex-wrap gap-3">
              {canFounderDecide && (
                <>
                  <button
                    onClick={() => onAction?.(startupId, request, "approved")}
                  ...
                  <button
                    onClick={() => onAction?.(startupId, request, "rejected")}
                    className="px-6 py-2.5 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 text-xs font-bold text-white transition-colors"
                  >
                    Reject
                  </button>
                </>
              ...
              {canWithdraw && (
                <button
                  onClick={() => onAction?.(startupId, request, "withdrawn")}
                  className="px-6 py-2.5 rounded-xl bg-red-500/10 border border-red-500/20 hover:bg-red-500/20 text-xs font-bold text-red-400 transition-colors"
                >
                  Withdraw Request
                </button>
              )}
...
function CreateStartupForm({ onClose, onSuccess }) {
  ...
  return (
    <motion.form
      ...
      exit={{ opacity: 0 }}
      onSubmit={onSubmit}
      className="p-6 md:p-8 rounded-[2rem] bg-[#0B0D10]/80 border border-white/5 shadow-2xl space-y-6 max-w-3xl"
    >
      <div>
        <h2 className="text-2xl font-black text-white mb-1">Add New Startup</h2>
        <p className="text-sm text-slate-400 font-medium">
          Create a verifiable profile to attract smart capital and mentors.
        </p>
      </div>

      {error && (
        <div className="p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-200 text-sm font-bold">
          {error}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div>
          ...
          <input
            className={inputClass}
            placeholder="Official BR Number"
            value={formData.BR}
            onChange={(e) =>
      ...
      <UploadField
        label="Company Logo or Banner"
        accept="image/png,image/jpeg,image/webp"
        onChange={(e) => setPhotoFile(e.target.files?.[0] || null)}
        files={photoFile ? [photoFile] : []}
        onClear={() => setPhotoFile(null)}
        helperText="Landscape image under 2MB"
        icon="image"
      />
